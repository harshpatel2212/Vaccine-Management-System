...
// global varibales -  as many functions of different class use them and are also used in main. So these are kept global
...
// to compute faster with directly having data of particular ID(object of Storage class) vaccination - true is taken, false if not then.
...
{
    int  int_day = 0, int_month = 0, int_year = 0;
    const int MAXLEN = 80;
    char s[MAXLEN];
    time_t t = time(0);
